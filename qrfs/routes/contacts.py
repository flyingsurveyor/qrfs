...
def contacts_view():
    ...
    if request.method == "POST":
        ...
        if not public_key_input and key_qr and key_qr.filename:
            temp_key_qr_path = os.path.join(
                current_app.config["TEMP_DIR"], f"contact_import_{secure_filename(key_qr.filename)}"
            )
            ...
            try:
                ...
                if lower.endswith(".pdf"):
                    ...
                    with open(temp_key_qr_path, encoding="utf-8") as f:
        ...
        try:
            ...
            signing_part = (
                f", signing {contact['signing_key_id']}" if contact.get("signing_key_id") else ""
            )
            flash(
                (
                    f"Contact saved: {contact['name']} "
                    f"(enc {contact['encryption_key_id']}{signing_part})."
                ),
                "success",
            )
        except Exception as exc:
