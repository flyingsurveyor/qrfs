...
from ..core.key_utils import (
    build_identity_card_payload,
    build_public_key_qr_payload,
    qr_png_data_uri,
)
...
def _json_download(payload: dict | bytes, filename: str):
    if isinstance(payload, dict):
        ...
        data = payload
    return send_file(
        io.BytesIO(data), as_attachment=True, download_name=filename, mimetype="application/json"
    )
    ...
    if request.method == "POST":
        ...
        if action == "export_backup":
            ...
            if backup_password and backup_password != backup_password_confirm:
                ...
                return redirect(url_for("keys.keys_view"))
            try:
                payload = export_identity_backup(
                    base_dir, master_pw, backup_password=backup_password or None
                )
        ...
        if action == "import_backup":
            ...
            new_master_password_confirm = (
                request.form.get("new_master_password_confirm") or ""
            ).strip()
        ...
        if action == "import_recovery":
            ...
            master_password_new_confirm = (
                request.form.get("recovery_master_password_confirm") or ""
            ).strip()
