...
from ..core.chunker import parse_chunk, reconstruct_from_chunks
from ..core.crypto_utils import decrypt_file_payload_auto, inspect_crypto_blob
from ..core.keystore import get_public_identity_no_password, identity_exists, unlock_identity
...
def _cleanup_old_decode_tasks() -> None:
    ...
    with DECODE_TASKS_LOCK:
        stale = [
            task_id for task_id, task in DECODE_TASKS.items() if task.get("updated_at", 0) < cutoff
        ]
...
def _decode_progress_updater(
    task_id: str, stage: str, start_percent: int, end_percent: int, noun: str
):
...
def _collect_chunk_payloads_from_files(
    scan_pdf, scan_images, config: dict
...
def _collect_chunk_payloads_from_paths(
    scan_pdf_path: str | None, scan_image_paths: list[str], task_id: str | None = None
) -> tuple[list[bytes], dict]:
    decode_stats = _build_empty_decode_stats()
    chunk_payloads: list[bytes] = []
    ...
    if scan_pdf_path and os.path.isfile(scan_pdf_path):
        if task_id:
            _update_decode_task(
                ...
                percent=12,
                stage="PDF scan",
                message="Reading QR pages from the uploaded PDF…",
            )
        pdf_chunks, pdf_stats = decode_qr_bytes_from_pdf(
            scan_pdf_path,
            return_stats=True,
            progress_callback=_decode_progress_updater(task_id, "PDF scan", 12, 68, "page")
            if task_id
            else None,
        )
        chunk_payloads.extend(pdf_chunks)
    ...
    if scan_image_paths:
        ...
        if task_id:
            _update_decode_task(
                ...
                percent=image_start,
                stage="Image scan",
                message="Reading QR pages from uploaded images…",
            )
        img_chunks, img_stats = decode_qr_bytes_from_images(
            scan_image_paths,
            return_stats=True,
            progress_callback=_decode_progress_updater(
                task_id, "Image scan", image_start, image_end, "image"
            )
            if task_id
            else None,
        )
...
def _enrich_signature_info(
    base_dir: str, my_identity: dict | None, signature_info: dict | None
) -> dict | None:
    if not signature_info:
        return None
    enriched = dict(signature_info)
    if my_identity and signature_info.get("signer_fingerprint") == my_identity.get(
        "signing_fingerprint"
    ):
...
def _decode_result_context(
    recovered: dict,
    output_name: str,
    file_id_hex: str | None,
    encryption_mode: str,
...
def _complete_decode_from_blob(
    base_dir: str,
    temp_dir: str,
    output_dir: str,
    has_identity: bool,
    my_identity: dict | None,
    encrypted_blob: bytes,
    fec_info: dict,
    decode_stats: dict,
    qr_count: int,
    file_id_hex: str | None,
    pending_template: str = "decode.html",
):
...
def _stage_decode_uploads(
    scan_pdf, scan_images, config: dict, task_id: str
...
def _run_decode_task(task_id: str, payload: dict) -> None:
    ...
    try:
        ...
        _update_decode_task(
            ...
            percent=5,
            stage="Preparing",
            message="Opening the uploaded files…",
        )
        chunk_payloads, decode_stats = _collect_chunk_payloads_from_paths(
            scan_pdf_path, scan_image_paths, task_id=task_id
        )
        ...
        _update_decode_task(
            ...
            message="Reassembling the QRFS blob from the detected chunks…",
        )
        ...
        _update_decode_task(
            ...
            percent=95,
            stage="Inspection",
            message="Checking whether the reconstructed blob is cleartext or protected…",
        )
        crypto_info = inspect_crypto_blob(encrypted_blob)
        ...
        _update_decode_task(
            ...
            message=(
                "Blob reconstructed. Open the unlock dialog to enter "
...
def decode_view():
    ...
    try:
        scan_pdf_path, scan_image_paths, task_dir = _stage_decode_uploads(
            scan_pdf, scan_images, current_app.config, task_id
        )
    except Exception as exc:
        _cleanup_decode_uploads(
            os.path.join(current_app.config["TEMP_DIR"], f"decode_task_{task_id}")
        )
        _update_decode_task(
            task_id,
            status="error",
            percent=100,
            stage="Error",
            message="Unable to stage uploads.",
            error=str(exc),
        )
...
def rescue_view():
    ...
    chunk_payloads, decode_stats = _collect_chunk_payloads_from_files(
        scan_pdf, scan_images, current_app.config
    )
