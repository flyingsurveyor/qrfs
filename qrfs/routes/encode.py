...
from ..core.pdfgen import PER_PAGE, build_png_zip, build_qr_pdf
from ..core.qrdecode import decode_qr_bytes_from_images, decode_qr_bytes_from_pdf
from ..core.utils import human_bytes, percent_str, timestamp_slug
...
def _cleanup_old_tasks() -> None:
    ...
    with ENCODE_TASKS_LOCK:
        stale = [
            task_id for task_id, task in ENCODE_TASKS.items() if task.get("updated_at", 0) < cutoff
        ]
...
def _pdf_progress_updater(task_id: str, start_percent: int = 28, end_percent: int = 92):
    def _callback(done: int, total: int):
        pct = (
            start_percent
            if total <= 0
            else start_percent + int((done / total) * (end_percent - start_percent))
        )
        _update_task(
            task_id,
            status="running",
...
def _run_encode_task(task_id: str, payload: dict) -> None:
    try:
        ...
        _update_task(
            ...
            message="Packaging content…",
        )
        packed = pack_file_payload(
            filename=safe_name, mime_type=mime_type, file_bytes=file_bytes, compress=compress
        )
        ...
        _update_task(
            task_id,
            percent=12,
            stage="Content protection",
            message="Encrypting or signing the payload…",
        )
        if encryption_mode == "clear":
            encrypted = encrypt_file_payload_clear(
                packed, sender_signing_private_key_b64=signing_private_key or None
            )
        elif encryption_mode == "pubkey":
            encrypted = encrypt_file_payload_pubkey(
                packed, public_key, sender_signing_private_key_b64=signing_private_key or None
            )
        else:
        ...
        _update_task(
            task_id, percent=20, stage="Chunking", message="Splitting the blob into QR codes…"
        )
        chunks = make_chunks(
            encrypted,
        ...
        estimate = estimate_encode_sizes(
            len(file_bytes),
            len(packed),
            len(encrypted),
            chunk_size,
        ...
        _update_task(
            ...
            percent=28,
            stage="PDF creation",
            message=f"Generating the PDF ({total_pages} pages)…",
        )
        build_qr_pdf(
            chunks,
            output_pdf,
        ...
        if generate_png_zip:
            _update_task(
                task_id, percent=92, stage="Optional PNG", message="Also generating the PNG ZIP…"
            )
            build_png_zip(
                chunks,
                output_png_zip,
        ...
        _update_task(
            task_id, percent=99, stage="Manifest", message="Saving the technical manifest…"
        )
        ...
        result = {
            ...
            "compression_gain": human_bytes(len(file_bytes) - len(packed))
            if len(packed) < len(file_bytes)
            else "none",
...
def voice_view():
    ...
    return render_template(
        "voice.html", contacts=contacts, has_identity=has_id, my_identity=my_identity
    )
...
def photo_view():
    ...
    my_identity = get_public_identity_no_password(base_dir)
    return render_template(
        "photo.html", contacts=contacts, has_identity=has_id, my_identity=my_identity
    )
...
def encode_view():
    ...
    if request.method == "GET":
        return render_template(
            "encode.html", contacts=contacts, has_identity=has_id, my_identity=my_identity
        )
    ...
    if (
        fec_type == "rs"
        and fec_group_size
        and (fec_parity_count < 1 or fec_parity_count >= fec_group_size)
    ):
        flash(
            (
                "For Reed-Solomon, parity QR per group must be at least 1 and "
                "smaller than the number of data QR in the group."
            ),
            "error",
        )
        return _redirect_back(return_to)
    ...
    if encryption_mode == "pubkey":
        if contact_id == "__self__" and has_id:
            ...
            if contact:
                public_key = (
                    contact.get("encryption_public_key_b64") or contact.get("public_key_b64") or ""
                ).strip()
    ...
    return jsonify(
        {
            ...
            "result_url": url_for("encode.encode_result_view", task_id=task_id)
