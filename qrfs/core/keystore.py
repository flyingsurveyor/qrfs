...
def export_identity_backup(
    base_dir: str, master_password: str, backup_password: str | None = None
) -> bytes:
...
def import_identity_backup(
    base_dir: str,
    backup_blob: bytes | str,
    backup_password: str,
...
def import_identity_from_private_keys(
    base_dir: str,
    *,
    ...
    if identity_exists(base_dir) and not overwrite:
        raise ValueError(
            "A local identity already exists. Enable overwrite to import the private keys."
        )
    _validate_master_password(master_password)
    ...
    identity = _build_identity(
        display_name=(display_name or "").strip(), enc_sk=enc_sk, sign_sk=sign_sk
    )
    _save_identity(base_dir, identity, master_password)
    return _public_info(identity)


def import_identity_recovery_sheet(
    base_dir: str, recovery_blob: bytes | str, *, master_password: str, overwrite: bool = False
) -> dict:
