...
def _normalize_contact(item: dict[str, Any]) -> dict[str, Any]:
    ...
    if enc_b64:
        ...
        contact["encryption_key_id"] = (
            contact.get("encryption_key_id")
            or contact.get("key_id")
            or key_id_from_public_key(enc_raw).hex()
        )
        contact["key_id"] = contact["encryption_key_id"]
        contact["encryption_fingerprint"] = contact.get("encryption_fingerprint") or contact.get(
            "fingerprint_sha256"
        )
    ...
    if sign_b64:
        ...
        if len(sign_raw) == 32:
            ...
            contact["signing_key_id"] = (
                contact.get("signing_key_id") or signing_key_id_from_verify_key(sign_raw).hex()
            )
            contact["signing_fingerprint"] = contact.get(
                "signing_fingerprint"
            ) or signing_fingerprint_from_verify_key(sign_raw)
...
def add_contact(
    base_dir: str, *, name: str, public_key_input: str, note: str = ""
) -> dict[str, Any]:
    ...
    existing = next(
        (
            ...
            or item.get("encryption_public_key_b64") == contact["encryption_public_key_b64"]
        ),
        None,
    )
    if existing:
...
def find_contact_by_signer(
    base_dir: str, *, signer_key_id: str | None = None, signer_fingerprint: str | None = None
) -> dict[str, Any] | None:
    ...
    for item in load_contacts(base_dir):
        ...
        if (
            signer_fingerprint
            and (item.get("signing_fingerprint") or "").lower() == signer_fingerprint
        ):
            return item
    return None
