...
def parse_chunk(raw: bytes) -> Chunk:
    magic = raw[:4]
    if magic != MAGIC_V3:
        raise ValueError("QR payload has an invalid chunk magic header.")
    version = raw[4]
    ...
    if len(raw) < start + header_len + 2:
        raise ValueError(
            "QRC3 chunk too short: expected at least "
            f"{start + header_len + 2} bytes, got {len(raw)}."
        )
    kind, fec_type, index, total, payload_len, group_index, group_size, parity_count = (
        struct.unpack(">BBIIHIHH", raw[start : start + header_len])
    )
...
def make_chunks(
    ...
    *,
    ...
    if fec_type == "rs" and fec_group_size and fec_parity_count >= fec_group_size:
        raise ValueError(
            "With RS, parity must be smaller than the number of data chunks per group."
        )
    ...
    for gi in range(math.ceil(total / fec_group_size)):
        ...
        if fec_type == "rs":
            output.extend(
                _build_rs_parity_chunks(
                    file_id, gi, total, group, chunk_size, fec_parity_count, fec_group_size
                )
            )
        else:
            output.append(
                _build_xor_parity_chunk(file_id, gi, total, group, chunk_size, fec_group_size)
            )
    return output
...
def _recover_group_xor(
    group_chunks: list[Chunk], total_data: int, chunk_size: int, nominal_group_size: int
) -> dict[int, bytes]:
    ...
    expected_indexes = [
        start_index + i for i in range(len(lengths)) if start_index + i < total_data
    ]
...
def _recover_group_rs(
    ...
    parity_chunks = sorted(
        [c for c in group_chunks if c.kind == KIND_PARITY], key=lambda c: c.parity_index
    )
