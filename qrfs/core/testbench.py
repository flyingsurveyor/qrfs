...
def build_synthetic_payload(size: int, seed: int, pattern: str = "mixed") -> bytes:
    ...
    while len(out) < size:
        ...
        if mode == 0:
            ...
            sentence = f"QRFS seed={seed} offset={len(out)} pattern=mixed ".encode()
            out.extend(sentence)
        else:
...
def run_single_test(
    *,
    ...
    try:
        ...
        packed_restored, _, recipient_key_id, signature_info = decrypt_file_payload_auto(
            rebuilt_encrypted, **decode_kwargs
        )
    ...
    result.fec_info = (result.fec_info or {}) | {
        "fec_type": fec_type,
        "fec_parity_count": fec_parity_count,
    }
    return result
